...
// если до этого был запущен nginx, то порт 80 будет занят и apache не запустится
// 1 вариант: надо остановить nginx
...
// прочитать все из таблицы user (\G для удобного просмотра)
SELECT * FROM user\G;
