...
clear
Ctrl + L
ctrl + U - очистка строки слева
ctrl + К - очистка строки справа

reset - перезагружает терминал (стирает историю команд)
...
# чтобы создать сразу несколько папок в одном каталоге
mkdir dir1 dir2 dir3

# чтобы создать сразу несколько вложенных папок
mkdir dir1/dir2/dir3 -p

# просмотреть файловую систему в виде дерева
...
(-N номер строки, -I поиск будет регистронезависимым, -р поиск)

•	выведем первую строку из файла (если без 1, будет выведено первые 10 строк)
...
# открыть на определенной строке и столбце
nano +200,15 mciri_poem.txt
...
# создадим жесткую ссылку на него (backup)
ln original_file hardlink

# создадим мягкую ссылку на него (ярлык)
ln -s original_file softlink

# восстановить комнды по номеру в history
!183

# Резюме про ссылки:
* мягкая ссылка опирается на имя файла
* жесткая ссылка опирается на номер файла (inode)
* при копировании файла меняется его inode
* для работы с файлом можно использовать жесткую ссылку
* файл считается удаленным, если не него не ссылается ни одна жесткая ссылка
